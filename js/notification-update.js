$(document).ready(function () {
  ...
  setInterval(() => {
    if (/login.php/i.test(window.location.href.toString())) {
      $.post("../../Sanitary_Shop/gestione-update-login-home.php", { action: "get-info" }, (data) => {
        ...
        if (Object.keys(data_parse).length > 0) {
          ...
          if (notifiche.length > 0) {
            ...
            $("#icona_notifiche > img").attr(
              "alt",
              "hai nuove notifiche, cliccare per accedere allo storico delle notifiche"
            );
          } else {
            $("#box-notifiche").html(
              '<li> \
  ...
  function switchUserIcon() {
    $.post(
      "../../Sanitary_Shop/gestione-update-login-home.php",
      { action: "get-count-notifiche" },
      (data) => {
        const data_parse = JSON.parse(data);
        if (Object.keys(data_parse).length > 0) {
          if (data_parse["numero_notifiche"] > 0) {
            $("#login_link svg").html(
              '<title id="login-icon">' +
                data_parse["title"] +
                '</title> \
